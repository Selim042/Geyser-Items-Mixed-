...
public class EnumHelperBukkit {
	...
	private static void setup() {
		...
		try {
			...
			GeyserItemsSpigot.LOGGER.log(Level.SEVERE, "Error setting up EnumHelper.", e);
		}
	...
	/*
	 * Everything below this is found at the site below, and updated to be able
	 * to compile in Eclipse/Java 1.6+ Also modified for use in decompiled code.
	 * Found at: http://niceideas.ch/roller2/badtrash/entry/
	...
	private static <T extends Enum<?>> T makeEnum(Class<T> enumClass, @Nullable String value,
			int ordinal, Class<?>[] additionalTypes, @Nullable Object[] additionalValues)
			throws Exception {
	...
	public static void setFailsafeFieldValue(Field field, @Nullable Object target,
	...
	@Nullable
	private static <T extends Enum<?>> T addEnum(boolean test, final Class<T> enumType,
			@Nullable String enumName, final Class<?>[] paramTypes, @Nullable Object[] paramValues) {
		if (!isSetup) {
			setup();
		}
		...
		for (Field field : fields) {
			...
			if (name.equals("$VALUES") || name.equals("ENUM$VALUES")) // Added
																		// 'ENUM$VALUES'
																		// because
																		// Eclipse's
																		// internal
		...
		int flags =
				// (FMLForgePlugin.RUNTIME_DEOBF ? Modifier.PUBLIC :
				Modifier.PRIVATE
						// )
		...
		if (valuesField == null) {
			...
			for (Field field : fields) {
				if ((field.getModifiers() & flags) == flags
						&& field.getType().getName().replace('.', '/').equals(valueType)) // Apparently
																							// some
																							// JVMs
																							// return
																							// .'s
		...
		if (valuesField == null) {
			...
			// lines.add(String.format("Runtime Deobf: %s",
			...
			for (Field field : fields) {
				String mods = String.format("%16s", Integer.toBinaryString(field.getModifiers()))
						.replace(' ', '0');
				lines.add(String.format("       %s %s: %s", mods, field.getName(),
						field.getType().getName()));
			}

			for (String line : lines)
				GeyserItemsSpigot.LOGGER.log(Level.SEVERE, line);

			if (test) {
				throw new RuntimeException(
						"Could not find $VALUES field for enum: " + enumType.getName());
			}
			return null;
		}
		...
		try {
			...
			GeyserItemsSpigot.LOGGER.log(Level.SEVERE, "Error adding enum with EnumHelper.", e);
